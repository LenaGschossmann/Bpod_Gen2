...
%}
function ConfigureBonsaiSocket_2WayUDP
global BpodSystem
...
BpodSystem.GUIHandles.BonsaiPortEdit = uicontrol('Style', 'edit', 'String', '50555', 'Position', [75 115 150 30], 'FontSize', 14, 'FontWeight', 'bold', 'ForegroundColor', 'k', 'BackgroundColor', [.9 .9 .9]);
...
if isempty(BpodSystem.BonsaiSocket)
    ...
    % Connect on one port for reading (Matlab acts as server)
    PortAsServer = str2num(BpodSystem.GUIHandles.BonsaiPortEdit.String);
    try
        BpodSystem.BonsaiSocket = UDPCom(PortAsServer, 'server');
    catch
        set(BpodSystem.GUIHandles.BonsaiConnectStatus, 'String', 'Disconnected', 'ForegroundColor', 'r');
        rethrow(lasterror);
    end
    
    PortAsClientSource = str2num(BpodSystem.GUIHandles.BonsaiPortEdit.String) - 1;
    PortAsClientSink = str2num(BpodSystem.GUIHandles.BonsaiPortEdit.String) + 1;
    try % Connect on one port for sending (Matlab as client)
        BpodSystem.BonsaiSocketClient = UDPCom(PortAsClientSink, 'client', PortAsClientSource);
    catch
        set(BpodSystem.GUIHandles.BonsaiConnectStatus, 'String', 'Disconnected', 'ForegroundColor', 'r');
        rethrow(lasterror);
    end
    ...
    set(BpodSystem.GUIHandles.BonsaiConnectButton, 'CData', BpodSystem.GUIHandles.BonsaiDisconnectButtonGFX);
     ...
     BpodSystem.BonsaiSocketClient = [];

     disp('UDP connections closed.')
